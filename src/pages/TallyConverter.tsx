...
import { ArrowLeft, FileCode2, UploadCloud, CheckCircle2, FileSpreadsheet, FileText, X } from 'lucide-react';
...
export default function TallyConverter({ onBack }: TallyConverterProps) {
  ...
  const processTallyData = async (xmlTarget: File, htmlTarget: File) => {
    try {
      const res = await fetch(`http://${window.location.hostname}:3001/api/usage/increment`, {
        method: 'POST',
