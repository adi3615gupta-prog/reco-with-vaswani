...
import { parseFile, detectColumnMapping, mapToRecords, type ColumnMapping, type InvoiceRecord } from '@/lib/fileParser';
import { reconcile, type ReconciliationResult, type MatchStatus } from '@/lib/reconciliation';
  ...
  const handleFetchTally = async () => {
    ...
    try {
      ...
      const records: InvoiceRecord[] = allData.map(v => {
        return {
          supplierName: v.partyName || v.vchType,
          gstin: v.partyGstin,
          invoiceNo: v.reference || v.vchNo,
          invoiceDate: v.date,
          igst: v.igst,
          cgst: v.cgst,
          sgst: v.sgst,
          source: 'PR',
          taxableValue: v.totalAmount - (v.igst + v.cgst + v.sgst)
        };
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="glass-card-np p-6 rounded-2xl border border-slate-800 space-y-6 relative overflow-hidden">
                 ...
                 {!twoBFile ? (
                     <FileUploadZone 
                       onUpload={handleTwoBUpload} 
                       title="Upload GSTR-2B (Excel)" 
