 ...
 *     ₹10,00,001+             → 30%
 */
export function getAllTaxBracketRules(fy: FinancialYear): TaxBracketRule[] {
  ruleId = 0; // Reset for idempotency
  const rules: TaxBracketRule[] = [];
  ...
  for (const age of [AgeCategory.NORMAL, AgeCategory.SENIOR, AgeCategory.SUPER_SENIOR]) {
    rules.push(
      makeRule(RegimeType.NEW, age, 0, 400000, 0, fy),
      makeRule(RegimeType.NEW, age, 400001, 800000, 5, fy),
      makeRule(RegimeType.NEW, age, 800001, 1200000, 10, fy),
      makeRule(RegimeType.NEW, age, 1200001, 1600000, 15, fy),
      makeRule(RegimeType.NEW, age, 1600001, 2000000, 20, fy),
      makeRule(RegimeType.NEW, age, 2000001, 2400000, 25, fy),
  ...
  rules.push(
    makeRule(RegimeType.OLD, AgeCategory.NORMAL, 0, 250000, 0, fy),
    makeRule(RegimeType.OLD, AgeCategory.NORMAL, 250001, 500000, 5, fy),
    makeRule(RegimeType.OLD, AgeCategory.NORMAL, 500001, 1000000, 20, fy),
    makeRule(RegimeType.OLD, AgeCategory.NORMAL, 1000001, Infinity, 30, fy),
  );

  // ── OLD REGIME — Senior (60-79) ───────────────────────────
  rules.push(
    makeRule(RegimeType.OLD, AgeCategory.SENIOR, 0, 300000, 0, fy),
    makeRule(RegimeType.OLD, AgeCategory.SENIOR, 300001, 500000, 5, fy),
    makeRule(RegimeType.OLD, AgeCategory.SENIOR, 500001, 1000000, 20, fy),
    makeRule(RegimeType.OLD, AgeCategory.SENIOR, 1000001, Infinity, 30, fy),
  );

  // ── OLD REGIME — Super Senior (80+) ───────────────────────
  rules.push(
    makeRule(RegimeType.OLD, AgeCategory.SUPER_SENIOR, 0, 500000, 0, fy),
    makeRule(RegimeType.OLD, AgeCategory.SUPER_SENIOR, 500001, 1000000, 20, fy),
...
// ═══════════════════════════════════════════════════════════════════
// DEDUCTION STATUTORY LIMITS (Chapter VI-A + others)
// ═══════════════════════════════════════════════════════════════════

export interface DeductionLimitConfig {
  sectionCode: string;
  /** Maximum deduction allowed in ₹. Infinity means no cap. */
  maxLimit: number;
  /** Description for audit trail */
  description: string;
  /** Whether this deduction is allowed under New Regime */
  allowedInNewRegime: boolean;
  /**
   * Some sections share an aggregate cap (e.g., 80C + 80CCC + 80CCD1 = ₹1.5L).
   * This field groups them.
   */
 ...
 */
export const DEDUCTION_LIMITS: DeductionLimitConfig[] = [
 ...
 */
export function getDeductionLimit(sectionCode: string): DeductionLimitConfig | undefined {
...
export interface SurchargeBracket {
  /** Income threshold above which this rate applies */
  incomeThreshold: number;
  /** Surcharge rate as percentage */
  rate: number;
}
 ...
 */
export const SURCHARGE_BRACKETS_OLD: SurchargeBracket[] = [
 ...
 */
export const SURCHARGE_BRACKETS_NEW: SurchargeBracket[] = [
  { incomeThreshold: 2_00_00_000, rate: 25 },
  { incomeThreshold: 1_00_00_000, rate: 15 },
  { incomeThreshold: 50_00_000, rate: 10 },
];
...
export interface Rebate87AConfig {
  /** Maximum total taxable income to be eligible */
  incomeThreshold: number;
 ...
 */
export const REBATE_87A_NEW: Rebate87AConfig = {
  incomeThreshold: 12_00_000,
  maxRebate: 60_000,
};

export const REBATE_87A_OLD: Rebate87AConfig = {
 ...
 */
export const AGGREGATE_CAPS: Record<string, number> = {
