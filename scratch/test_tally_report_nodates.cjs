...
async function test() {
  try {
    console.log("Fetching Ledger Vouchers report without dates...");
    const start = Date.now();
    const res = await sendTallyRequest(xml);
    console.log("Time taken:", Date.now() - start, "ms. Size:", res.length);
    fs.writeFileSync('scratch_res_report_nodates.xml', res);
    console.log("Saved to scratch_res_report_nodates.xml");
  } catch (err) {
    console.error(err);
  }
}
