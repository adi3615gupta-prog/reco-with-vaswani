...
exports.mapToRecords = mapToRecords;
exports.numVal = numVal;
exports.exportToXlsx = exportToXlsx;
exports.deriveOverallStatus = deriveOverallStatus;
exports.exportMonthlyComparison = exportMonthlyComparison;
exports.exportPartyWise = exportPartyWise;
exports.exportPartyMismatchSheets = exportPartyMismatchSheets;
...
function mapToRecords(rows, mapping, source, sourceLabel) {
    const safeNum = (val) => {
    ...
    return rows.map((row) => {
        ...
        if (mapping.taxableValue)
            rec.taxableValue = safeNum(row[mapping.taxableValue]);
        if (mapping.filingStatus)
            rec.filingStatus = String(row[mapping.filingStatus] || '');
        if (mapping.filingDate)
            rec.filingDate = String(row[mapping.filingDate] || '');
        return rec;
    });
}
const STATUS_STYLES = {
...
const DEFAULT_STYLE = { headerFill: '1E3A5F', headerFont: 'FFFFFF', rowFill: 'FFFFFF' };
const PARTY_STATUS_FILL = {
    'All Matched': 'E6F5ED',
    'Has Mismatches': 'FEF3C7',
    'Has Missing': 'FEE2E2',
};
const PARTY_STATUS_HEADER = {
    'All Matched': '1B7A4D',
...
function autoFitCols(headers, data) {
...
function addCorporateHeader(ws, colCount, companyName, reportName) {
    ...
    if (hasHomeBtn) {
        ...
        ws[homeAddr] = {
            ...
            v: '🏠 RETURN TO MENU',
            l: {
                Target: `#'Navigation'!A1`,
                Tooltip: 'Click to go back to Main Dashboard Menu',
            },
        };
        ws[homeAddr].s = {
...
function appendExecutiveSummary(wb, companyName, reportName, stats, breakdown) {
...
function applySheetStyles(ws, style, rowCount, opts) {
...
function escapeSheetNameForHyperlink(sheetName) {
    return sheetName.replace(/'/g, "''");
}
function makeHyperlinkCell(sheetName, displayText) {
    const escapedName = escapeSheetNameForHyperlink(sheetName);
    return {
        t: 's',
        v: displayText,
        l: {
            Target: `#'${escapedName}'!A4`,
            Tooltip: `Navigate to ${sheetName}`,
        },
    };
}
function appendNavigationSheet(wb, sheetNames, companyName) {
    ...
    for (let c = 0; c < 3; c++) {
        ...
        if (ws[addr])
            ws[addr].s = { fill: { fgColor: { rgb: '1E293B' } }, font: { bold: true, color: { rgb: 'FFFFFF' }, sz: 10 }, alignment: { vertical: 'center', horizontal: 'center' }, border: { bottom: { style: 'medium', color: { rgb: '000000' } } } };
    }
    const topAddr = XLSX.utils.encode_cell({ r: 3, c: 0 });
    if (ws[topAddr])
        ws[topAddr].s = { fill: { fgColor: { rgb: '0F172A' } }, font: { bold: true, color: { rgb: '38BDF8' }, sz: 12 }, alignment: { vertical: 'center', horizontal: 'center' } };
    if (!ws['!merges'])
        ws['!merges'] = [];
...
function buildSheetRows(records) {
    ...
    // Collect all record keys and preserve insertion order
    const allKeys = [];
    records.forEach(r => Object.keys(r).forEach(k => { if (!allKeys.includes(k))
        allKeys.push(k); }));
    // Compute extra columns excluding baseCols (by exact match)
    const extraCols = allKeys.filter(k => !baseCols.includes(k));
    // Merge base + extra but dedupe by trimmed key to avoid near-duplicates like trailing spaces
    const seen = new Set();
    const cols = [];
    ...
    // Robust value getter: prefer exact key, then trimmed-key match
    const getVal = (r, col) => {
        if (col in r)
            return r[col];
    ...
    const data = records.map((r) => cols.map((c) => getVal(r, c) ?? ''));
    return { cols, data };
}
function numVal(v) {
    if (typeof v === 'number')
...
function exportToXlsx(results, filename, companyName) {
    ...
    const dateCols = allCols.reduce((acc, key, i) => (key.includes('Date') ? [...acc, i] : acc), []);
    const numberCols = allCols.reduce((acc, key, i) => ((key.includes('GST') && !key.includes('GSTIN')) || key.includes('Diff') || key.includes('Tax') || key.includes('Value') ? [...acc, i] : acc), []);
    const allGstDiffColIdx = allCols.indexOf('GST Diff');
    ...
    const getL = (idx) => XLSX.utils.encode_col(idx);
    for (let i = 0; i < allData.length; i++) {
        const rowNum = i + 4;
        if (allGstDiffColIdx >= 0) {
            allData[i][allGstDiffColIdx] = { t: 'n', f: `ABS(${getL(igstPrIdx)}${rowNum}-${getL(igst2bIdx)}${rowNum})+ABS(${getL(cgstPrIdx)}${rowNum}-${getL(cgst2bIdx)}${rowNum})+ABS(${getL(sgstPrIdx)}${rowNum}-${getL(sgst2bIdx)}${rowNum})`, z: '#,##0.00;[Red]-#,##0.00' };
        }
    }
    const allWs = XLSX.utils.aoa_to_sheet([]);
    XLSX.utils.sheet_add_aoa(allWs, [allCols, ...allData], { origin: 'A3' });
    addCorporateHeader(allWs, allCols.length, companyName, 'All Records');
    applySheetStyles(allWs, DEFAULT_STYLE, results.length, {
        startRow: 2,
        dateCols,
        numberCols,
        colWidths: autoFitCols(allCols, allData),
    });
    allWs['!views'] = [{ state: 'frozen', xSplit: 0, ySplit: 3, topLeftCell: 'A4', showGridLines: false }];
    allWs['!autofilter'] = { ref: XLSX.utils.encode_range({ s: { r: 2, c: 0 }, e: { r: results.length + 2, c: allCols.length - 1 } }) };
    for (let r = 1; r <= results.length; r++) {
        const status = String(allData[r - 1][0] || 'Other');
        const style = STATUS_STYLES[status] || DEFAULT_STYLE;
        const isEven = r % 2 === 0;
        const fallbackFill = isEven ? DEFAULT_STYLE.rowFill : 'FFFFFF';
        const fillRgb = style.rowFill !== 'FFFFFF' ? style.rowFill : fallbackFill;
        for (let c = 0; c < allCols.length; c++) {
            const addr = XLSX.utils.encode_cell({ r: r + 2, c });
            if (allWs[addr] && allWs[addr].s) {
                allWs[addr].s.fill = { fgColor: { rgb: fillRgb } };
            }
        }
    }
    ...
    for (const status of Object.keys(grouped)) {
        ...
        const dateCols = cols.reduce((acc, key, i) => (key.includes('Date') ? [...acc, i] : acc), []);
        const numberCols = cols.reduce((acc, key, i) => ((key.includes('GST') && !key.includes('GSTIN')) || key.includes('Diff') || key.includes('Tax') || key.includes('Value') ? [...acc, i] : acc), []);
        ...
        for (let i = 0; i < data.length; i++) {
            const rowNum = i + 4;
            if (gstDiffCol >= 0)
                data[i][gstDiffCol] = { t: 'n', f: `ABS(${getL(ip)}${rowNum}-${getL(ib)}${rowNum})+ABS(${getL(cp)}${rowNum}-${getL(cb)}${rowNum})+ABS(${getL(sp)}${rowNum}-${getL(sb)}${rowNum})`, z: '#,##0.00;[Red]-#,##0.00' };
        }
        const ws = XLSX.utils.aoa_to_sheet([]);
        ...
        applySheetStyles(ws, style, rows.length, {
            ...
            dateCols,
            numberCols,
            colWidths: autoFitCols(cols, data),
        });
    ...
    if (suggested.length > 0) {
        const wsSugg = XLSX.utils.aoa_to_sheet([]);
        XLSX.utils.sheet_add_json(wsSugg, suggested, { origin: 'A3' });
    ...
    // ---- GSTIN Conflicts: Detect same GSTIN mapped to multiple party names (PR/2B)
    const gstinMap = new Map();
    for (const r of results) {
        const gstinPR = String(r['GSTIN (PR)'] || '').trim();
        const gstin2B = String(r['GSTIN (2B)'] || '').trim();
        const prName = String(r['Supplier Name (PR)'] || '').trim();
        const tbName = String(r['Supplier Name (2B)'] || '').trim();
        const addTo = (key, src) => {
            if (!gstinMap.has(key))
                gstinMap.set(key, { prNames: new Set(), tbNames: new Set(), count: 0 });
            const entry = gstinMap.get(key);
            if (src === 'pr' && prName)
                entry.prNames.add(prName);
            if (src === 'tb' && tbName)
                entry.tbNames.add(tbName);
            entry.count++;
        };
        if (gstinPR)
            addTo(gstinPR, 'pr');
    ...
    for (const [gstin, info] of gstinMap.entries()) {
        ...
        if (!needsReview && prCount <= 1 && tbCount <= 1)
            continue;
        let suggested = 'Review';
        if (info.tbNames.size === 1 && info.prNames.size > 1)
            suggested = 'Prefer GSTR2B name';
        else if (info.prNames.size === 0 && info.tbNames.size >= 1)
            suggested = 'Add to Books (from GSTR2B)';
        gstConflicts.push({
            'GSTIN': gstin,
            'Parties in Books': [...info.prNames].join(' | ') || '—',
            'Parties in GSTR2B': [...info.tbNames].join(' | ') || '—',
            'Records Count': info.count,
            'Suggested Action': suggested,
        });
    }
    if (gstConflicts.length > 0) {
        const wsGst = XLSX.utils.aoa_to_sheet([]);
        XLSX.utils.sheet_add_json(wsGst, gstConflicts, { origin: 'A3' });
        addCorporateHeader(wsGst, 5, companyName, 'GSTIN Conflicts');
        applySheetStyles(wsGst, { headerFill: '8B5CF6', headerFont: 'FFFFFF', rowFill: 'FFFFFF' }, gstConflicts.length, {
            startRow: 2,
            colWidths: [{ wch: 18 }, { wch: 35 }, { wch: 35 }, { wch: 12 }, { wch: 20 }]
        });
        wsGst['!views'] = [{ state: 'frozen', xSplit: 0, ySplit: 3, topLeftCell: 'A4', showGridLines: false }];
        XLSX.utils.book_append_sheet(wb, wsGst, 'GSTIN Conflicts');
...
function deriveOverallStatus(statuses) {
    if (statuses.has('Mismatch') || statuses.has('Name Mismatch') || statuses.has('Possible Match') || statuses.has('Name Matched (No GSTIN)') || statuses.has('Value Mismatch'))
        return 'Has Mismatches';
    if (statuses.has('Not in 2B') || statuses.has('Missing in 2B') || statuses.has('Unmatched Vendor') || statuses.has('Not in Books') || statuses.has('Missing in PR'))
        return 'Has Missing';
    return 'All Matched';
}
function appendPartyWiseSheets(wb, results, companyName) {
    const map = new Map();
    ...
    for (const r of results) {
        ...
        const normName = (0, reconciliation_1.normalizePartyName)(party);
        ...
        if (gstin && normName && nameIndex.has(normName) && nameIndex.get(normName) !== gstin) {
            ...
            if (existingKey.startsWith('NAME::') || existingKey.startsWith('UNKNOWN::')) {
                ...
                if (existing) {
                    ...
                    p.prCgst += existing.prCgst;
                    p.prSgst += existing.prSgst;
                    p.prIgst += existing.prIgst;
                    p.cgst2B += existing.cgst2B;
                    p.sgst2B += existing.sgst2B;
                    p.igst2B += existing.igst2B;
    ...
    const parties = Array.from(map.values()).sort((a, b) => (a.party || a.gstin).localeCompare(b.party || b.gstin));
    ...
    const detailRows = [];
    const partyAnchorRow = []; // 1-indexed Excel row of each party's header
    const merges = [];
    const subtotalRowIdxs = new Set();
    const emptyRowIdxs = new Set();
    for (const p of parties) {
        const overall = deriveOverallStatus(p.statuses);
        const partyHeaderRowNumber = detailRows.length + 4; // actual Excel row number of this party header
        const partyHeaderRowIndex = partyHeaderRowNumber - 1; // zero-based sheet row index for merges
        partyAnchorRow.push(partyHeaderRowNumber);
        ...
        merges.push({ s: { r: partyHeaderRowIndex, c: 0 }, e: { r: partyHeaderRowIndex, c: detailHeaders.length - 3 } });
        ...
        const invStartRow = partyHeaderRowNumber + 1;
        const invEndRow = invStartRow + p.invoices.length - 1;
        const subRowNumber = invEndRow + 1;
        for (let col = 4; col <= 9; col++) {
            const colLetter = XLSX.utils.encode_col(col);
            subRow[col] = hasInv ? { t: 'n', f: `SUM(${colLetter}${invStartRow}:${colLetter}${invEndRow})`, z: '0.00' } : 0;
        }
        subRow[10] = { t: 's', f: `"Diff: ₹" & TEXT(ABS(E${subRowNumber}-F${subRowNumber})+ABS(G${subRowNumber}-H${subRowNumber})+ABS(I${subRowNumber}-J${subRowNumber}), "0.00")` };
        merges.push({ s: { r: subRowNumber - 1, c: 0 }, e: { r: subRowNumber - 1, c: 3 } });
        subtotalRowIdxs.add(subRowNumber - 1);
        detailRows.push(subRow);
        emptyRowIdxs.add(subRowNumber);
...
function exportMonthlyComparison(rows, filename, debitNotes, companyName) {
    const wb = XLSX.utils.book_new();
    // Add Executive Summary
    const counts = {};
    let totalDiff = 0;
    ...
    for (const r of rows) {
        ...
        if (gstin && normName && nameIndex.has(normName) && nameIndex.get(normName) !== gstin) {
            ...
            if (existingKey.startsWith('NAME::') || existingKey.startsWith('UNKNOWN::')) {
                ...
                if (existing) {
                    ...
                    pAgg.cgst2B += existing.cgst2B;
                    pAgg.sgst2B += existing.sgst2B;
                    pAgg.igst2B += existing.igst2B;
        ...
        if (!p.gstin && gstin)
            p.gstin = gstin;
        p.prCgst += num(r.cgstTally);
        p.prSgst += num(r.sgstTally);
        p.prIgst += num(r.igstTally);
        p.cgst2B += num(r.cgstCmp);
        p.sgst2B += num(r.sgstCmp);
        p.igst2B += num(r.igstCmp);
    }
    const pwHeaders = [
        'GST No.', 'Party Name',
        'PR CGST', 'PR SGST', 'PR IGST',
    ...
    const partyList = Array.from(partyMap.values()).sort((a, b) => (a.party || a.gstin).localeCompare(b.party || b.gstin));
    const pwData = partyList.map((p, i) => [
        p.gstin, p.party,
        +p.prCgst.toFixed(2), +p.prSgst.toFixed(2), +p.prIgst.toFixed(2),
        +p.cgst2B.toFixed(2), +p.sgst2B.toFixed(2), +p.igst2B.toFixed(2),
        { t: 'n', f: `C${i + 4}-F${i + 4}`, z: '#,##0.00;[Red]-#,##0.00' },
        { t: 'n', f: `D${i + 4}-G${i + 4}`, z: '#,##0.00;[Red]-#,##0.00' },
        { t: 'n', f: `E${i + 4}-H${i + 4}`, z: '#,##0.00;[Red]-#,##0.00' },
    ]);
    ...
    // ---- Sheet 3: Monthly Tax Comparison (6-table FY layout w/ Debit Notes) ----
    {
        ...
        const fyIdx = (m) => (m >= 3 ? m - 3 : m + 9);
        const parseMonthFY = (s) => {
            if (!s)
                return -1;
            const str = String(s).trim();
            let m = str.match(/^(\d{1,2})[-/](\d{1,2})[-/](\d{2,4})$/);
            if (m)
                return fyIdx(parseInt(m[2], 10) - 1);
            m = str.match(/^(\d{4})[-/](\d{1,2})[-/](\d{1,2})$/);
            if (m)
                return fyIdx(parseInt(m[2], 10) - 1);
            const d = new Date(str);
            if (!isNaN(d.getTime()))
                return fyIdx(d.getMonth());
            return -1;
        };
        const mk = () => Array.from({ length: 12 }, () => ({ cgst: 0, sgst: 0, igst: 0 }));
        const prGross = mk(), tbGross = mk(), prDN = mk(), tbDN = mk();
        for (const r of rows) {
            const pi = parseMonthFY(r.dateTally);
            if (pi >= 0) {
                prGross[pi].cgst += numVal(r.cgstTally);
                prGross[pi].sgst += numVal(r.sgstTally);
                prGross[pi].igst += numVal(r.igstTally);
            }
            const ti = parseMonthFY(r.dateCmp);
            if (ti >= 0) {
                tbGross[ti].cgst += numVal(r.cgstCmp);
                tbGross[ti].sgst += numVal(r.sgstCmp);
                tbGross[ti].igst += numVal(r.igstCmp);
            }
        }
        ...
        const sub = (a, b) => a.map((x, i) => ({ cgst: x.cgst - b[i].cgst, sgst: x.sgst - b[i].sgst, igst: x.igst - b[i].igst }));
        ...
        const ROWS = 33;
        const SHIFT = 2;
        const grid = Array.from({ length: ROWS + SHIFT }, () => Array(COLS).fill(null));
        const merges = [];
        const colStarts = [0, TABLE_W + GAP, (TABLE_W + GAP) * 2]; // 0, 6, 12
        const fillTable = (titleRow, hdrRow, dataStart, totalRow, startCol, title, data) => {
            titleRow += SHIFT;
            hdrRow += SHIFT;
            dataStart += SHIFT;
            totalRow += SHIFT;
        ...
        const mtcWs = XLSX.utils.aoa_to_sheet(grid);
        ...
        mtcWs['!cols'] = Array.from({ length: COLS }, (_, c) => {
            // gap cols
            if (c === TABLE_W || c === TABLE_W * 2 + GAP)
                return { wch: 3 };
            const off = c < TABLE_W ? c : c < TABLE_W * 2 + GAP ? c - TABLE_W - GAP : c - TABLE_W * 2 - GAP * 2;
            return { wch: off === 0 ? 12 : 10 };
        });
        const titleFillsTop = { 'Purchase': '1E3A5F', 'AS PER GSTR-2B': '0D7A5F', 'DIFFERENCE': 'B45309' };
        const titleFillsBot = { 'PURCHASE': '1E3A5F', 'DEBIT NOTE': 'B45309', 'TOTAL PURCHASE AS PER TALLY': '0D7A5F' };
        const isGapCol = (c) => c === TABLE_W || c === TABLE_W * 2 + GAP;
        const tableForCol = (c) => c < TABLE_W ? 0 : c < TABLE_W * 2 + GAP ? 1 : 2;
        const colOffsetIn = (c) => c - colStarts[tableForCol(c)];
        const sectionMap = (r) => {
            const shiftedR = r - SHIFT;
            if (shiftedR >= 0 && shiftedR <= 15)
                return { mainRow: 0 + SHIFT, titleRow: 1 + SHIFT, hdrRow: 2 + SHIFT, totalRow: 15 + SHIFT, titles: ['Purchase', 'AS PER GSTR-2B', 'DIFFERENCE'] };
            if (shiftedR >= 17 && shiftedR <= 32)
                return { mainRow: 17 + SHIFT, titleRow: 18 + SHIFT, hdrRow: 19 + SHIFT, totalRow: 32 + SHIFT, titles: ['PURCHASE', 'DEBIT NOTE', 'TOTAL PURCHASE AS PER TALLY'] };
            return null;
        };
        for (let r = 0; r < ROWS + SHIFT; r++) {
            ...
            for (let c = 0; c < COLS; c++) {
                const addr = XLSX.utils.encode_cell({ r, c });
                if (r < SHIFT)
                    continue; // corporate header rows
    ...
    // ---- Sheet 4: Monthly Tax Comparison (FY layout: 2B | JV+Purchase | Difference) ----
    {
        ...
        const SHIFT = 2;
        const grid = Array.from({ length: rowsCount + SHIFT }, () => Array(COLS).fill(null));
        const tables = [
        ...
        const mWs = XLSX.utils.aoa_to_sheet(grid);
        ...
        const titleFills = { '2B': '0D7A5F', 'JV+Purchase': '1E3A5F', 'Difference': 'B45309' };
...
// --- Party-wise Report ---
function exportPartyWise(parties, filename, companyName, statusFilter) {
    const filteredParties = statusFilter && statusFilter.length > 0
        ? parties
            ...
            const totals = invoices.reduce((acc, inv) => {
                ...
                if (inv.status === 'Perfect Match' || inv.status === 'Matched' || inv.status === 'Matched (Rounded)' || inv.status === 'Matched (Diff Date)')
                    acc.perfectMatch += 1;
                if (inv.status === 'Value Mismatch' || inv.status === 'Mismatch')
                    acc.valueMismatch += 1;
                if (inv.status === 'Not in 2B' || inv.status === 'Missing in 2B')
                    acc.invoiceMissing += 1;
                if (inv.status === 'Unmatched Vendor')
                    acc.unmatchedVendor += 1;
                if (inv.status === 'Not in Books' || inv.status === 'Missing in PR')
                    acc.missingInPR += 1;
                return acc;
            }, {
    ...
    const summaryData = parties.map((p) => {
        const igstDiff = +(p.totals.igstPR - p.totals.igst2B).toFixed(2);
        const cgstDiff = +(p.totals.cgstPR - p.totals.cgst2B).toFixed(2);
        const sgstDiff = +(p.totals.sgstPR - p.totals.sgst2B).toFixed(2);
        const totalDiff = +(Math.abs(igstDiff) + Math.abs(cgstDiff) + Math.abs(sgstDiff)).toFixed(2);
        return [
            ...
            p.totals.igstPR, p.totals.igst2B, igstDiff,
            p.totals.cgstPR, p.totals.cgst2B, cgstDiff,
            p.totals.sgstPR, p.totals.sgst2B, sgstDiff,
            totalDiff,
            p.overall,
        ];
    });
    const totalSummary = parties.reduce((acc, p) => {
        acc.count += p.totals.count;
        acc.perfectMatch += p.totals.perfectMatch;
        acc.valueMismatch += p.totals.valueMismatch;
        acc.invoiceMissing += p.totals.invoiceMissing;
        acc.unmatchedVendor += p.totals.unmatchedVendor;
        acc.missingInPR += p.totals.missingInPR;
        acc.igstPR += p.totals.igstPR;
        acc.igst2B += p.totals.igst2B;
        acc.cgstPR += p.totals.cgstPR;
        acc.cgst2B += p.totals.cgst2B;
        acc.sgstPR += p.totals.sgstPR;
        acc.sgst2B += p.totals.sgst2B;
        return acc;
    }, {
        count: 0, perfectMatch: 0, valueMismatch: 0, invoiceMissing: 0,
        unmatchedVendor: 0, missingInPR: 0,
        igstPR: 0, igst2B: 0, cgstPR: 0, cgst2B: 0, sgstPR: 0, sgst2B: 0,
    });
    const totalsRow = [
        'GRAND TOTAL', '', totalSummary.count,
        totalSummary.perfectMatch, totalSummary.valueMismatch,
        totalSummary.invoiceMissing, totalSummary.unmatchedVendor, totalSummary.missingInPR,
        totalSummary.igstPR, totalSummary.igst2B, +(totalSummary.igstPR - totalSummary.igst2B).toFixed(2),
        totalSummary.cgstPR, totalSummary.cgst2B, +(totalSummary.cgstPR - totalSummary.cgst2B).toFixed(2),
        totalSummary.sgstPR, totalSummary.sgst2B, +(totalSummary.sgstPR - totalSummary.sgst2B).toFixed(2),
        +(Math.abs(totalSummary.igstPR - totalSummary.igst2B) + Math.abs(totalSummary.cgstPR - totalSummary.cgst2B) + Math.abs(totalSummary.sgstPR - totalSummary.sgst2B)).toFixed(2),
        '',
    ];
    ...
    for (const p of parties) {
        ...
        const headerRow = Array(NUM_DETAIL_COLS).fill('');
        ...
        const subRow = Array(NUM_DETAIL_COLS).fill('');
        subRow[0] = 'SUBTOTAL';
        const invStartRow = startRowIdx + 2;
        const invEndRow = invStartRow + p.invoices.length - 1;
        const subRowNumber = invEndRow + 1;
        for (let col = 4; col <= 9; col++) {
            const colLetter = XLSX.utils.encode_col(col);
            subRow[col] = hasInvoiceRows ? { t: 'n', f: `SUM(${colLetter}${invStartRow}:${colLetter}${invEndRow})`, z: '#,##0.00;[Red]-#,##0.00' } : 0;
        }
        subRow[10] = { t: 's', f: `"Diff: ₹" & TEXT(ABS(E${subRowNumber}-F${subRowNumber})+ABS(G${subRowNumber}-H${subRowNumber})+ABS(I${subRowNumber}-J${subRowNumber}), "0.00")` };
    ...
    appendNavigationSheet(wb, ['Party Summary', 'Party Details'], companyName);
    XLSX.writeFile(wb, filename);
}
function exportPartyMismatchSheets(results, filename, companyName) {
    const wb = XLSX.utils.book_new();
    const sheetNames = [];
    // 1. Filter out only the mismatches/missing records
    const mismatchedResults = results.filter((r) => {
        const status = String(r.status || '');
        // We EXCLUDE perfect matches so we only get "Not in 2B", "Value Mismatch", etc.
        return !['Perfect Match', 'Matched', 'Matched (Rounded)', 'Matched (Diff Date)'].includes(status);
    });
    // 2. Group by Party Name
    ...
    for (const r of mismatchedResults) {
        ...
        if (gstin && normName && nameIndex.has(normName) && nameIndex.get(normName) !== gstin) {
            ...
            if (existingKey !== gstin && (existingKey.startsWith('NAME::') || existingKey.startsWith('UNKNOWN::'))) {
                const existing = partiesMap.get(existingKey);
                if (existing) {
                    let p = partiesMap.get(gstin);
                    if (!p) {
                        p = { partyName: existing.partyName || party, gstin, records: [] };
                        partiesMap.set(gstin, p);
                    }
                    p.records.push(...existing.records);
                    if (!p.partyName && existing.partyName)
                        p.partyName = existing.partyName;
                    partiesMap.delete(existingKey);
                }
                nameIndex.set(normName, gstin);
            }
            key = gstin;
        }
        let p = partiesMap.get(key);
        if (!p) {
            p = { partyName: party, gstin, records: [] };
            partiesMap.set(key, p);
            if (normName && (!nameIndex.has(normName) || nameIndex.get(normName).startsWith('NAME::'))) {
                nameIndex.set(normName, key);
            }
        }
        if (!p.partyName && party)
            p.partyName = party;
        if (!p.gstin && gstin)
            p.gstin = gstin;
        p.records.push(r);
    ...
    const parties = Array.from(partiesMap.values())
        .sort((a, b) => (a.partyName || a.gstin).localeCompare(b.partyName || b.gstin));
    if (parties.length === 0) {
        const ws = XLSX.utils.aoa_to_sheet([['No Mismatches Found. Everything is perfectly matched!']]);
        XLSX.utils.book_append_sheet(wb, ws, 'No Mismatches');
        XLSX.writeFile(wb, filename);
        return;
    }
    const seenSheetNames = new Set();
    // 3. Create a dedicated sheet for each party
    for (const party of parties) {
        const partyName = party.partyName || 'Unknown';
        // Excel limits sheet names to 31 chars and forbids certain special characters
        const sheetLabel = party.gstin ? `${partyName} | ${party.gstin}` : partyName;
        const safeName = sheetLabel
        ...
        const recordsForSheet = party.records.map((r) => {
            const pr = r.prRecord;
        ...
        const { cols, data } = buildSheetRows(recordsForSheet);
        const gstDiffColIdx = cols.indexOf('GST Diff');
        const igstPrIdx = cols.indexOf('IGST (PR)');
        const igst2bIdx = cols.indexOf('IGST (2B)');
        const cgstPrIdx = cols.indexOf('CGST (PR)');
        const cgst2bIdx = cols.indexOf('CGST (2B)');
        const sgstPrIdx = cols.indexOf('SGST (PR)');
        const sgst2bIdx = cols.indexOf('SGST (2B)');
        const getL = (idx) => XLSX.utils.encode_col(idx);
        for (let i = 0; i < data.length; i++) {
            const rowNum = i + 4;
            if (gstDiffColIdx >= 0) {
                data[i][gstDiffColIdx] = { t: 'n', f: `ABS(${getL(igstPrIdx)}${rowNum}-${getL(igst2bIdx)}${rowNum})+ABS(${getL(cgstPrIdx)}${rowNum}-${getL(cgst2bIdx)}${rowNum})+ABS(${getL(sgstPrIdx)}${rowNum}-${getL(sgst2bIdx)}${rowNum})`, z: '#,##0.00;[Red]-#,##0.00' };
            }
        }
        const dateCols = cols.reduce((acc, key, i) => (key.includes('Date') ? [...acc, i] : acc), []);
        const numberCols = cols.reduce((acc, key, i) => ((key.includes('GST') && !key.includes('GSTIN')) || key.includes('Diff') || key.includes('Tax') || key.includes('Value') ? [...acc, i] : acc), []);
        const ws = XLSX.utils.aoa_to_sheet([]);
        XLSX.utils.sheet_add_aoa(ws, [cols, ...data], { origin: 'A3' });
        // Insert header and apply standard formatting/AutoFilters
        addCorporateHeader(ws, cols.length, companyName, partyName.substring(0, 50));
        applySheetStyles(ws, DEFAULT_STYLE, recordsForSheet.length, {
            startRow: 2,
            dateCols,
            numberCols,
            colWidths: autoFitCols(cols, data),
        });
        ws['!autofilter'] = { ref: XLSX.utils.encode_range({ s: { r: 2, c: 0 }, e: { r: recordsForSheet.length + 2, c: cols.length - 1 } }) };
        XLSX.utils.book_append_sheet(wb, ws, uniqueName);
    }
    // Add a Navigation index at the very beginning so users can click through parties
    appendNavigationSheet(wb, sheetNames, companyName);
    XLSX.writeFile(wb, filename);
}
